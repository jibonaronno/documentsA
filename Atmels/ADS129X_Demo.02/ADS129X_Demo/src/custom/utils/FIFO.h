 ...
 * this buffer should be sized appropriately to accommodate the data.
 *
...
/**
 * @brief Obtains a pointer item from the FIFO buffer.
 *
 * This function return a pointer to the element at the fifo buffer front.
 * This function affects the read pointer and the stored items counter.
 *
 * @param fifo Pointer to a fifo_descriptor structure.
 *
 * @return  Returns the pointer to the location that holds the item in
 * fifo buffer if there is data available on the fifo buffer, if the buffer
 ...
 * @brief Obtains a pointer item from the FIFO buffer.
 *
 * This function return a pointer to the element at the fifo buffer at the
 * specified index.
 * This function does not performs any changes on the FIFO's descriptor.
 *
 * @param fifo Pointer to a fifo_descriptor structure.
 *
 * @return  Returns the pointer to the location that holds the item in
 * fifo buffer at the specified index, if the buffer hasn't enough items
 * this returns NULL.
 */
 ...
 * the side and the amount of discarded data depends on the parameters passed.
 *
 * @param fifo Pointer to a fifo_descriptor structure.
 * @param count The number of elements to discard from the buffer.
 * @param side Defines if data should be discarded from the front or back side
 * of the buffer.
 *
 * @return Returns true if the data was discarded, false if not.
 */
bool fifo_discard(fifo_t fifo, uint32_t count, enum fifo_side side);

/**
 * @brief Gets the number of items inside the fifo.
 *
 * Gets the number of items inside the fifo.
 *
 * @param fifo Pointer to a fifo_descriptor structure.
 *
 * @return This function the number of items inside the fifo, if empty
 * returns 0.
 */
