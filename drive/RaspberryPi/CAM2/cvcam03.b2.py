...
class PyMotive(object):
	def __init__(self):
		...
		self.ftp_ip = "ftp.toiconnect.net"
		self.ftp_usr = "jibon@toiconnect.net"
		self.ftp_pwd = "jibon@2021"
		self.ftp_client = FTP(self.ftp_ip)
		self.ftp_client.login(user=self.ftp_usr, passwd = self.ftp_pwd)
		try:
			p = subprocess.Popen(["./pppip.sh"], stdout=subprocess.PIPE)
			sts = os.waitpid(p.pid, 0)
			...
			if b'NA' in stdout_value:
				...
				sleep(5)
				pppip = str(stdout_value)
				cmds = 'sudo route add default gw %s' % pppip
		...
		while(True):
			...
			try:
				...
				if stDev > self.sdThresh:
					...
					self.filename = '/home/pi/pycam/cam2/x00000002_%s.jpg' % timestampMessage
					fname = 'x00000002_%s.jpg' % timestampMessage
					...
					try:
						self.ftp_client = FTP(self.ftp_ip)
						self.ftp_client.login(user=self.ftp_usr, passwd = self.ftp_pwd)
						file_stream = open(self.filename,"rb")         # read file to send to byte
						self.ftp_client.storbinary("{CMD} {FileName}".format(CMD="STOR",FileName=fname),file_stream)     
						file_stream.close()
						self.ftp_client.close
					except self.ftp_client.all_errors:
						print("FTP ERROR")
					print('end')
