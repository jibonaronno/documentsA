...
static double a[EMDN/2][EMDN/2]={{0.0}};
...
void imf(double *x,double *h,double *x1,double *y1,double *x2,double *y2,double *outh)
{
    ...
    **/
    ...
    for (int imfcount = 0; imfcount <2; imfcount ++)
    {
        ...
        } while(abs(SD) > 2000/*0.01*/);//计算出一次imf
        SD = 0.0;
        /*计算残余分量*/
...
void emd(double* pInputnum, int numLength, double* pOutputData)
{
    ...
    /*********************EMD处理数据END******************/
    /* r为分解后的本征模函数*/
    imf(x,y,x1,y1,x2,y2,pOutputData);
