...
void Init_Modbus( void )
{
  ...
  sMb.Pit[eMB_PIT_F06_SET0].pwPit = (UINT16 *)&sDsp.wIpAddr;                // Setting IpAddress ~ Reset
  sMb.Pit[eMB_PIT_F06_SET0].wSta  = 1029;
  sMb.Pit[eMB_PIT_F06_SET0].wEnd  = 1037;
// 1038~1039(ilUtc)가 32bit라 F06으로 쓰면 안됨 시간이라 문제가 될 수 있음
  sMb.Pit[eMB_PIT_F06_SET1].pwPit = (UINT16 *)&sDsp.wIpAddr;                // Setting IpAddress ~ Reset
...
//        SciaRegs.SCICTL2.bit.TXINTENA = 1;      // TX Int
  ...
  else if( pMb->bRxData[7] == eMB_FUNC06 )
  {
    ...
    else if( i_wSta >= pMb->Pit[eMB_PIT_F06_SET1].wSta && i_wSta <= pMb->Pit[eMB_PIT_F06_SET1].wEnd )
    {
      ...
      i_pwData[0]  = ( (UINT16)pMb->bRxData[10] << 8 ) & 0xFF00;
      i_pwData[0] |= ( (UINT16)pMb->bRxData[11] << 0 ) & 0x00FF;
      if( i_wSta == 0 )   Conv_MramWrXCs( (UINT16 *)&pMram->wTrig,          1,    (UINT16 *)&sDsp.wTrig,      (UINT16 *)&pMram->wTrigXCs,     (UINT16 *)&sDsp.wTrigXCs    );
      if( i_wSta == 1 )   Conv_MramWrXCs( (UINT16 *)&pMram->wDebo,          1,    (UINT16 *)&sDsp.wDebo,      (UINT16 *)&pMram->wDeboXCs,     (UINT16 *)&sDsp.wDeboXCs    );
      i_bSta = 1;
    }
    if( i_bSta )
    {
